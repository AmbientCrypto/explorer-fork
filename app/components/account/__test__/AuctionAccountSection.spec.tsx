...
test('dispatches auction parsed accounts from jsonParsed data', async () => {
...
test('hides generic tokens and domains tabs for auction accounts', () => {
    expect(showGenericAccountTabs({ parsed: bundleEscrow, program: 'auction' } as any, 'auction:bundleEscrowV2')).toBe(
        false
    );
});

test('renders bundle escrow v2 details', () => {
...
test('renders verifier page v2 entries', () => {
    const verifierPage = {
        info: {
            bundleEscrow: pubkey,
            entries: [
...
test('renders config policy v2 details', () => {
    const configPolicy = {
        info: {
