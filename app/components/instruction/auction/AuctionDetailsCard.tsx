    ...
    if (typeof value === 'object' && 'name' in value) {
        ...
        return (
            <>
                ...
                {named.value !== undefined ? <span className="text-muted ms-2">#{named.value}</span> : null}
            </>
        );
    }
    if (typeof value === 'object') {
        return <pre className="d-inline-block text-start mb-0 data-wrap">{JSON.stringify(value, null, 2)}</pre>;
    }
    return String(value);
}
